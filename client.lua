...
function pegarpacote()
	...
	then 
		...
		-- Limpa as animações antes de aplicar a nova
		ClearPedTasksImmediately(ped)
		Citizen.Wait(100) -- Pequena pausa para garantir que a animação foi limpa
		PickupPackage()
	else 
		ox_lib:notify({
            title = 'Reciclagem',
...
function PickupPackage()
    ...
    -- Aplica a animação de carregar caixa (idle loop)
    TaskPlayAnim(ped, "anim@heists@box_carry@", "idle", 8.0, -1, -1, 51, 0, false, false, false)
    
    -- Carrega o modelo da caixa
...
function DropPackage()
    ...
    -- Para todas as animações do jogador
    ClearPedTasks(ped)
    
    -- Remove o objeto anexado
    if carryPackage ~= nil then
